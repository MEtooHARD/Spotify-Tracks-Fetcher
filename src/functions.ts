...
import { Album, Artist, Category, Genres, ISO3166_1_Alpha_2, Locale, Paged, Result, SearchResult, SearchType, SpotifyToken, Track } from "./types";
...
export async function defaultFetch<T>(url: string): Promise<Result<T>> {
    return await tryCatch<T>(
        extractResponse<T>(
            fetch(url, {
                method: 'GET',
                headers: { Authorization: BearerToken() }
